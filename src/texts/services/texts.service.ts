...
export class TextsService {
  async getTimes(conversation: MyConversation, ctx: MyContext) {
    ...
    await ctx.reply(DIALOGS.MEETINGS.DATE.A3, {
      reply_markup: {
        keyboard: [
          ...chunk(
            ages.map((e) => ({ text: e })),
  ...
  async getSpecificDates(
    ...
    const days = eachDayOfInterval({
      ...
      .slice(0, 14);
    await ctx.reply(DIALOGS.MEETINGS.DATE.A8, {
      reply_markup: {
        keyboard: [
          ...chunk(
  ...
  async getDate(conversation: MyConversation, ctx: MyContext) {
    ...
    await ctx.reply(DIALOGS.MEETINGS.DATE.A1, {
      reply_markup: {
        keyboard: [
          ...chunk(
    ...
    await ctx.reply(DIALOGS.MEETINGS.DATE.A2, {
      reply_markup: {
        keyboard: [
          ...chunk(
    ...
    return set(dayDate, { minutes: 0, milliseconds: 0, seconds: 0, hours: 0 });
  }
